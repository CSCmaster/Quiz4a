...
height_data <- data.frame(
  Friend = rep(c(rep("Edward", num_friends), rep("Hugo", num_friends), rep("Lucy", num_friends)), each = 1),
  True_Height = rep(true_heights, times = 3),
  Observed_Height = c(observed_heights_edward, observed_heights_hugo, observed_heights_lucy),
  Measurement_Approach = rep(c(rep("Edward", num_friends), rep("Hugo", num_friends), rep("Lucy", num_friends)), each = 1)
)
...
plot_data <- data.frame(
  Friend = height_data$Friend,
  Observed_Height = height_data$Observed_Height,
  True_Height = height_data$True_Height
)

ggplot(plot_data, aes(x = True_Height, y = Observed_Height, color = Friend)) +
  geom_point() +
  geom_line() +
  labs(title = "Observed vs. True Heights",
       x = "True Height",
       y = "Observed Height",
       color = "Friend") +
  theme_minimal()
